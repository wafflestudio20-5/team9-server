...
def test_success_register(client: test.Client):
    user_data = test_data_utils.UserData.create_nth_user_data(1)
...
@pytest.mark.django_db
def test_success_login(client: test.Client, user1: test_data_utils.UserData):
...
@pytest.mark.django_db
def test_fail_login(client: test.Client, user1: test_data_utils.UserData):
